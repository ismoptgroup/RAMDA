...
model = ConvNet()
model.load_state_dict(torch.load(args.path+'Models/ConvNet_FashionMNIST_'+str(args.seed)+'.pt'))
...
optimizer_grouped_parameters = group_model(model=model, name="ConvNet", lambda_=args.lambda_)
...
for epoch in range(args.epochs):
    ...
    f = open(args.path+'Results/Presentation/'+args.optimizer+'_'+args.regularization+'_ConvNet_on_FashionMNIST_presentation_'+str(args.seed)+'.txt', 'w+')  
    ...
    if args.optimizer == "ProxSGD" or args.optimizer == "RMDA" or args.optimizer == "RAMDA":
        for i, r in enumerate(zip(lrs, momentums, validation_accuracies, unstructured_sparsities, structured_sparsities, weighted_structured_sparsities)):
            f.write("epoch:{:<5d}\tlearning rate:{:<20.15f}\tmomentum:{:<20.15f}\tvalidation accuracy:{:<20.15f}\tunstructured sparsity:{:<20.15f}\tstructured sparsity:{:<20.15f}\tweighted structured sparsity:{:<20.15f}".format((i+1), r[0], r[1], r[2], r[3], r[4], r[5])+'\n')
            
    elif args.optimizer == "ProxAdamW":
        for i, r in enumerate(zip(lrs, momentums, validation_accuracies, unstructured_sparsities, structured_sparsities, weighted_structured_sparsities)):
            f.write("epoch:{:<5d}\tlearning rate:{:<20.15f}\tmomentum:{:<20.15f},{:<20.15f}\tvalidation accuracy:{:<20.15f}\tunstructured sparsity:{:<20.15f}\tstructured sparsity:{:<20.15f}\tweighted structured sparsity:{:<20.15f}".format((i+1), r[0], r[1][0], r[1][1], r[2], r[3], r[4], r[5])+'\n')    

    f.close()
    
    f = open(args.path+'Results/ForPlotting/'+args.optimizer+'_'+args.regularization+'_Lin_on_FashionMNIST_forplotting_'+str(args.seed)+'.txt', 'w+')
    ...
    torch.save(model.state_dict(), args.path+'Saved_Models/'+args.optimizer+'_'+args.regularization+'_ConvNet_on_FashionMNIST_'+str(args.seed)+'.pt')
